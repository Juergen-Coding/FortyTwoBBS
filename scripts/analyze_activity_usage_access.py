#!/usr/bin/env python3
"""Analyze B5.9 activity, quota and usage fields.
...
def parse_args(
    ...
    parser = argparse.ArgumentParser(
        description=(
            "Analyze B5.9 activity, quota and usage accesses."
        )
    )
    parser.add_argument(
        "--format",
...
def category(field: str) -> str:
    if field in ACCOUNT_TIMELINE_FIELDS:
        return "account-timeline"

    if field in TIME_ACCOUNT_FIELDS:
        return "time-account"

    if field in VALUE_ACCOUNT_FIELDS:
        return "value-account"

    if field in TRANSFER_USAGE_FIELDS:
        return "transfer-usage"

    if field in COMMUNITY_USAGE_FIELDS:
        return "community-usage"

    raise ValueError(f"field outside B5.9: {field}")
...
def load_data() -> tuple[
    ...
    missing_fields = [
        field
        for field in B5_9_FIELDS
        if field not in ownership_by_field
    ]

    if missing_fields:
        raise ValueError(
            "B5.9 fields missing from ownership baseline: "
            + ", ".join(missing_fields)
        )
    ...
    if decided_fields:
        raise ValueError(
            "B5.9 unexpectedly contains fields already decided: "
            + ", ".join(sorted(decided_fields))
        )

    non_open_fields = {
        field
        for field in B5_9_FIELDS
        if ownership_by_field[field]["DECISION_STATUS"] != "open"
    }

    if non_open_fields:
        raise ValueError(
            "B5.9 fields are not open in the ownership baseline: "
            + ", ".join(sorted(non_open_fields))
        )

    selected_accesses = [
        row
        for row in access_rows
        if row["FIELD"] in B5_9_FIELDS
    ]
    ...
    missing_access_fields = set(B5_9_FIELDS) - access_fields

    if missing_access_fields:
        raise ValueError(
            "B5.9 fields missing from direct-access inventory: "
...
def print_summary(
    ...
    print("B5.9 activity and usage access analysis")
    print("=======================================")
    print(
    ...
    print(f"Fields analyzed:                  {len(B5_9_FIELDS)}")
    print(f"Direct access occurrences:        {len(access_rows)}")
    print("Fields already decided:           0")
    print(f"Fields still open:                {len(B5_9_FIELDS)}")
    print()
...
def print_details(
    ...
    field_order = {
        field: index
        for index, field in enumerate(B5_9_FIELDS)
    }
...
def main(
    ...
    try:
        ...
        print(
            f"Activity/usage analysis: FAILED: {exc}",
            file=sys.stderr,
        )
        return 1
