...
SETUP_SQL=$SCRIPT_DIR/0005_login_lookup_fixture_setup.sql
CLEANUP_SQL=$SCRIPT_DIR/0005_login_lookup_fixture_cleanup.sql
TEMP_BINARY=/tmp/fortytwo-authd-login-integration-test.$$
fixture_created=0
...
cleanup()
{
    ...
    if [ "$fixture_created" -eq 1 ]; then
        ...
        then
            printf 'WARNING: could not remove PostgreSQL login fixture\n' >&2
            status=1
        fi
    fi
    exit "$status"
