...
SETUP_SQL=$SCRIPT_DIR/0006_session_fixture_setup.sql
VERIFY_SQL=$SCRIPT_DIR/0006_session_fixture_verify.sql
CLEANUP_SQL=$SCRIPT_DIR/0006_session_fixture_cleanup.sql
TEMP_BINARY=/tmp/fortytwo-authd-session-integration-test.$$
fixture_created=0
...
cleanup()
{
    ...
    if [ "$fixture_created" -eq 1 ]; then
        ...
        then
            printf 'WARNING: could not remove PostgreSQL session fixture\n' >&2
            status=1
        fi
    fi
    exit "$status"
